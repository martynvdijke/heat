...
func (h *Handler) GetHeatCards(c *gin.Context) {
...
func (h *Handler) AddHeatCard(c *gin.Context) {
	var hc models.HeatCard
	if err := c.ShouldBindJSON(&hc); err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	_, err := h.S.DB.Exec("INSERT INTO heat_cards (racer_id, location, card_type, lap_added) VALUES (?, ?, ?, ?)",
...
func (h *Handler) MoveHeatCard(c *gin.Context) {
	var req struct {
	...
	_, err := h.S.DB.Exec("UPDATE heat_cards SET location = ? WHERE id = ?", req.Location, req.CardID)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) DeleteHeatCard(c *gin.Context) {
	idStr := c.Query("id")
...
func (h *Handler) ClearHeatCards(c *gin.Context) {
	racerIDStr := c.Query("racer_id")
	racerID, _ := strconv.Atoi(racerIDStr)
	if racerID > 0 {
		h.S.DB.Exec("DELETE FROM heat_cards WHERE racer_id = ?", racerID)
	} else {
		h.S.DB.Exec("DELETE FROM heat_cards")
	}
	c.Status(http.StatusOK)
}
...
func (h *Handler) GetGearShifts(c *gin.Context) {
	racerIDStr := c.Query("racer_id")
...
func (h *Handler) AddGearShift(c *gin.Context) {
	var gs models.GearShift
...
func (h *Handler) DeleteGearShift(c *gin.Context) {
	idStr := c.Query("id")
	id, err := strconv.Atoi(idStr)
	if err != nil || id <= 0 {
	...
	h.S.DB.Exec("DELETE FROM gear_shifts WHERE id = ?", id)
	c.Status(http.StatusOK)
}
...
func (h *Handler) SaveUpgradeCard(c *gin.Context) {
	var u models.UpgradeCard
...
func (h *Handler) DeleteUpgradeCard(c *gin.Context) {
	idStr := c.Query("id")
	id, err := strconv.Atoi(idStr)
	if err != nil || id <= 0 {
	...
	h.S.DB.Exec("DELETE FROM upgrade_cards WHERE id = ?", id)
	h.S.DB.Exec("DELETE FROM player_upgrades WHERE upgrade_id = ?", id)
	c.Status(http.StatusOK)
}

// Player Upgrades

func (h *Handler) GetPlayerUpgrades(c *gin.Context) {
	racerIDStr := c.Query("racer_id")
...
func (h *Handler) BuyUpgrade(c *gin.Context) {
	...
	_, err := h.S.DB.Exec("INSERT INTO player_upgrades (racer_id, upgrade_id, season_id, equipped, round_bought) VALUES (?, ?, ?, 1, ?)",
...
func (h *Handler) ToggleUpgrade(c *gin.Context) {
	var req struct {
		ID       int  `json:"id"`
	...
	_, err := h.S.DB.Exec("UPDATE player_upgrades SET equipped = ? WHERE id = ?", equipped, req.ID)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) DeletePlayerUpgrade(c *gin.Context) {
	idStr := c.Query("id")
	id, err := strconv.Atoi(idStr)
	if err != nil || id <= 0 {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "Invalid upgrade ID"})
	...
	h.S.DB.Exec("DELETE FROM player_upgrades WHERE id = ?", id)
	c.Status(http.StatusOK)
}
...
func (h *Handler) GetRacerLegendAbilities(c *gin.Context) {
	racerIDStr := c.Query("racer_id")
	racerID, _ := strconv.Atoi(racerIDStr)
...
func (h *Handler) AssignLegendAbility(c *gin.Context) {
	var req struct {
		RacerID   int `json:"racer_id"`
	...
	_, err := h.S.DB.Exec("INSERT OR IGNORE INTO racer_legend_abilities (racer_id, ability_id, active) VALUES (?, ?, 1)",
...
func (h *Handler) ToggleLegendAbility(c *gin.Context) {
	var req struct {
		ID     int  `json:"id"`
	...
	_, err := h.S.DB.Exec("UPDATE racer_legend_abilities SET active = ? WHERE id = ?", active, req.ID)
	if err != nil {
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) GetAvailableUpgradesForRacer(c *gin.Context) {
	racerIDStr := c.Query("racer_id")
	racerID, _ := strconv.Atoi(racerIDStr)
...
func (h *Handler) InitializeHeatDecks(c *gin.Context) {
	var req struct {
	...
	for _, racerID := range req.RacerIDs {
		...
		// Deal 3 to hand
		h.S.DB.Exec(`UPDATE heat_cards SET location = 'hand' WHERE racer_id = ? AND location = 'deck' LIMIT 3`, racerID)
	}
